...
from utils.utils import cold_start_result_save, result_save, result_save_i, test_result_save
...
def train(args, dataset, out_file, net_params):
    ...
    # Load dataset..
    trainset, testset, dataset_all = dataset.train_set_cold_start_spcancer, dataset.test_set_cold_start_spcancer, dataset.dataset_all
    ...
    if(args.Independent_Testing == True):
        ...
        for fold, (trainset_index, valset_index) in enumerate(folds):
            ...
            result = [epoch, test_loss, test_auc, test_aupr, test_f1, test_acc]
            cold_start_result_save(args, epoch, out_file, fold, result)
...
def main():
    ...
    # 结果保存路径
    out_file = args.out_dir + DATASET_NAME + '/' + out_dir_different_params + '_' + 'cold_start'
    # 如果不存在文件夹则创建文件夹
